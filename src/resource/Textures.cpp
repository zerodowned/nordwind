...
QSharedPointer<Textures::Entry> Textures::getEntry( ID _id, QSharedPointer<Hues::Entry> _hue ) {
	...
	QSharedPointer<Textures::Entry> result = Cache::instance().lookup( key );
	if(result.isNull()) {
		...
		if(!data.isEmpty()) {
			...
			result = Cache::instance().manage( new Textures::Entry( _id, decode( width, height, data, _hue), _hue) );
		}
	}
	return result;
}
...
Image Textures::decode( quint16 _width, quint16 _height, QByteArray _data, QSharedPointer<Hues::Entry> _hue ) {
	...
	stream.readRawData( static_cast<char*>(imgData.data()), sizeof(Colour16)*imgData.size() );
