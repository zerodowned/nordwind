...
void Huffman::in(QByteArray& data) {
	data = uncompress(data);
}

QByteArray Huffman::uncompress(QByteArray& data) {
	QByteArray uncompressed;
        unsigned char* pos = reinterpret_cast<unsigned char*>(data.data());
	...
	for(int i = 0;pos!=end;) {
		...
		mTreePos = tree[mTreePos * 2 + (mValue & mMask) ? 0 : 1];
		mMask >>= 1;             // shift on reck
		mBitNum++;

		if (mTreePos <= 0)       // this is a leaf.
		{
			if (mTreePos == -256)  // special flush character
			{
				mBitNum = 8;    // flush rest of byte
				mTreePos = 0;    // start on tree top again
				continue;
			}
			uncompressed[i++] = -mTreePos;   // data is negative value
			mTreePos = 0;      // start on tree top again
		}
	}
        return uncompressed;
