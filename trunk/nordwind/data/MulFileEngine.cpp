...
QAbstractFileEngine* MulFileEngineHandler::create(const QString& fileName) const {
	return QDir::match(QString("*.mul%1*").arg(MulFileEngine::separator), fileName) ? new MulFileEngine(
			fileName.section(MulFileEngine::separator, -2, -2),
			fileName.section(MulFileEngine::separator, -1, -1).toUInt()) : 0;
}
